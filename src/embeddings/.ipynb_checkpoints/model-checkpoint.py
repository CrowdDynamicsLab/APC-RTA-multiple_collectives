...
class MatrixFactorizationModel(CompletionModel):
  ...
  def prepare_item_factors(self, retrain=False):
    if retrain:
      ...
      np.save( os.path.join(self.foldername, "song_embeddings_%d" % self.emb_size), als_model.item_factors.to_numpy())

      print("if retrain is True a sparse binary train set must be given as input")
      
    else:
      self.item_factors = np.load( os.path.join(self.foldername, 'song_embeddings', self.emb_size))
