...
class DataManager():
  ...
  def __init__(self, foldername = "resources/data/", test_size=10000, min_songs_test=10, resplit=False, dim=128, 
    ...
    # Change all below to use os.path.join to make it platform independent

    self.song_embeddings_path = os.path.join(self.foldername, 'embeddings', f'song_embeddings_{dim}.npy')
    self.album_embeddings_path = os.path.join(self.foldername, 'embeddings', f'alb_embeddings_{dim}.npy')
    self.artist_embeddings_path = os.path.join(self.foldername, 'embeddings', f'art_embeddings_{dim}.npy')
    self.pop_embeddings_path = os.path.join(self.foldername, 'embeddings', f'pop_embeddings_{dim}.npy')
    self.dur_embeddings_path = os.path.join(self.foldername, 'embeddings', f'dur_embeddings_{dim}.npy')

    self.load_track_info()
    self.load_metadata()

    self.n_playlists = 10**6 if n_playlists is None else n_playlists
    self.n_tracks = 2262292 if n_tracks is None else n_tracks
  ...
  def load_playlist_track(self):
    self.playlist_track = load_npz(os.path.join(self.foldername, "rta_input", "playlist_track.npz"))
  
  def load_playlist_artist(self):
    self.playlist_artist = load_npz(os.path.join(self.foldername, "rta_input", "playlist_artist.npz"))
    # self.playlist_artist = load_npz("%s/rta_input/playlist_artist.npz" % self.foldername)

  def load_playlist_album(self):
    self.playlist_album = load_npz(os.path.join(self.foldername, "rta_input", "playlist_album.npz"))
  ...
  def load_metadata(self):
    ...
    self.song_album = np.load(os.path.join(self.foldername, "rta_input", "song_album.npy"))
    #self.song_album = np.load("%s/rta_input/song_album.npy" % self.foldername)
    self.song_artist = np.load(os.path.join(self.foldername, "rta_input", "song_artist.npy"))
    ...
    with open(os.path.join(self.foldername, "rta_input", "album_ids.pkl"), 'rb+') as f:
      self.album_ids = pickle.load(f)

    # with open("%s/rta_input/artist_ids.pkl" % self.foldername, 'rb+') as f:
    with open(os.path.join(self.foldername, "rta_input", "artist_ids.pkl"), 'rb+') as f:
      self.artist_ids = pickle.load(f)

    # with open("%s/rta_input/artist_songs.pkl" % self.foldername, 'rb+') as f:
    with open(os.path.join(self.foldername, "rta_input", "artist_songs.pkl"), 'rb+') as f:
      self.artist_songs = pickle.load(f)
  

    with open(os.path.join(self.foldername, 'rta_input', 'album_songs.pkl'), 'rb+') as f:
      self.album_songs = pickle.load(f)

    with open(os.path.join(self.foldername, 'rta_input', 'artist_names.pkl'), 'rb+') as f:
      self.artist_names = pickle.load(f)

    with open(os.path.join(self.foldername, 'rta_input', 'album_names.pkl'), 'rb+') as f:
  ...
  def load_track_info(self):
    ...
    with open(os.path.join(self.foldername, "rta_input", "tracks_info.json")) as f:
